    ...
    public function __construct()
    {
        ...
        if (substr($sysname, 0, 5) === "Linux") {
            ...
            if ($this->_exec("stty --version") === 0) {
                ...
                throw new phpSerialException("No stty availible, unable to run.");
            }
...
// 		     }
    ...
    public function deviceOpen($mode = "r+b")
    {
        ...
        $this->_dHandle = null;
        throw new phpSerialException("Unable to open the device");
    }
    ...
    public function deviceClose()
    {
        ...
        throw new phpSerialException("Unable to close the device");
    }
    ...
    public function confBaudRate($rate)
    {
        ...
        if (isset($validBauds[$rate])) {
            ...
            throw new phpSerialException("Unalbe to set baud rate: " . $rate);
    }
    ...
    {
        if ($this->_dState !== SERIAL_DEVICE_SET) {
            throw new phpSerialException("Unable to set length of a character : the device is either not set or opened");
        }
    ...
    {
        if ($this->_dState !== SERIAL_DEVICE_SET) {
            throw new phpSerialException("Unable to set the length of a stop bit : the device is either not set or opened");
        }
     ...
     */
    public function sendMessage($str, $waitForReply = 0.0)
    {
        $this->_buffer .= $str;
    ...
    private function _ckClosed()
    {
        if ($this->_dState !== SERIAL_DEVICE_CLOSED) {
            throw new phpSerialException("Device must be closed");
        }
